#!/usr/bin/env python3
"""
Demo: Submit an audio extraction job (video -> mp3).
...
def main():
    parser = argparse.ArgumentParser(description="Submit extract job")
    parser.add_argument("--video", required=True, help="Path to video file")
    args = parser.parse_args()

    print(f"Submitting extract job: video={args.video}")

    with open(args.video, "rb") as vf:
        resp = httpx.post(
            f"{GATEWAY}/api/v1/jobs",
            data={"job_type": "extract"},
            files={"video": vf},
            timeout=30,
        )
